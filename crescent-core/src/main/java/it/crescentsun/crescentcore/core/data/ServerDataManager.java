...
import static it.crescentsun.crescentcore.api.registry.CrescentNamespaceKeys.SERVER_KEY;
...
public class ServerDataManager {
    ...
    public ServerDataManager(CrescentCore crescentCore, DatabaseManager dbManager) {
    ...
    protected void createServerTable() {
        if (tableCreated) {
            crescentCore.getLogger().warning("Second attempt to create the server table. Ignoring...");
            return;
        }
        try (Connection connection = dbManager.getConnection()) {
            ...
            crescentCore.getLogger().info("Created " + SERVER_KEY + " table");
    ...
    protected void updateServerTable() {
        if (tableUpdated) {
            crescentCore.getLogger().warning("Second attempt to update the server table caught. Ignoring...");
            return;
        }
        try (Connection connection = dbManager.getConnection()) {
    ...
    public CompletableFuture<Void> incrementPlayersJoined() {
        ...
        return CompletableFuture.runAsync(() -> {
            try (Connection connection = dbManager.getConnection()) {
                ...
                crescentCore.getLogger().info("Hurray! A new player has joined the server!");
            } catch (SQLException e) {
                e.printStackTrace();
            }
        });
