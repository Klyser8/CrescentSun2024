...
import it.klynet.adventurousklynetmsg.KlyNetColor;
...
import it.klynet.klynetcore.core.data.ServerDataManager;
...
public class CrystalArtifact extends AsyncArtifact {
    ...
    public CompletableFuture<ItemStack> getItemAsync(int amount) {
        ServerDataManager serverDataManager = KlyNetCore.getInstance().getDatabaseManager().getServerDataManager();
        return serverDataManager.getCrystalsGenerated().thenApply(crystalsGenerated -> {
            ...
            List<Component> lore = new ArrayList<>();
            lore.add(MessageFormatter.gradientText(Component.text("Release, absorb."), KlyNetColor.PURPLE.getColor(), KlyNetColor.BLUE.getColor()));
