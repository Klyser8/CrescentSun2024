package it.klynet.crystals.crystalix;
...
import static it.klynet.klynetcore.api.registry.KlyNetNamespaceKeys.CRYSTALS_AMOUNT;
...
public class CrystalixEntity {
    ...
    public CrystalixEntity(Crystals plugin, Player owner) {
        this.plugin = plugin;
        this.owner = owner;
        ownerData = plugin.getKlyNetCore().getPlayerManager().getData(owner.getUniqueId());
    ...
    public Item spawnCrystalix() {
        Item crystalix = world.dropItem(location, ArtifactFactory.getItem(ArtifactNamespaceKeys.CRYSTALIX));
